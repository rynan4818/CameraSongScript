...
譜面フォルダ内のスクリプト、または `UserData/CameraSongScript/SongScripts` に置いたスクリプトを自動検出し、プレイ開始時にカメラへ適用します。さらに、汎用スクリプト、プレビュー再生、BetterSongListへのフィルタ・ソート追加、HttpSiraStatusでメニュー選択中とプレイ開始時の情報送信、未導入譜面の BeatSaver ダウンロードまでをひとまとめに扱えます。
...
- HttpSiraStatus へ、メニュー選択中とプレイ開始時の SongScript / CommonScript 状態と metadata を送信できます
...
| `CameraSongScript.HttpSiraStatus.dll` | HttpSiraStatus 連携。メニュー選択中とプレイ開始時の SongScript 情報を外部へ送信 |
...
スクリプトで動かす「対象のカメラ」をメニューで選択して、「専用カスタムシーン追加」で選択したカメラが１つある"CameraSongScript"カスタムシーンが追加されるので、「カスタムシーン」で選択します。

<img width="769" height="1002" alt="image" src="https://github.com/user-attachments/assets/24510f84-5937-41ab-8ed7-ffd66cefa795" />
...
`CameraSongScript.HttpSiraStatus.dll` を導入すると、メニュー選択中とプレイ開始時に `OtherJSON["CameraSongScript"]` へ現在状態のスナップショットを送信します。
...
`status` はその時点で分かっている現在状態を表します。`sceneContext=menu` では選択中の譜面や汎用スクリプト設定に応じた状態が送信され、`sceneContext=player` と `updateReason=playStart` ではプレイ開始時の確定状態が送信されます。

`Camera2` で `CommonScript=(Random)` を選んでいる場合、メニュー中は実際に使われる汎用スクリプトが未確定のため、`status=commonScript` かつ `isResolved=false` で送信されます。この場合、`scriptFileName` と `metadata` は空のままになり、プレイ開始時に確定値へ更新されます。
