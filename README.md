...
Beat Saber のカメラ Mod である [Camera2](https://github.com/kinsi55/CS_BeatSaber_Camera2) / [CameraPlus](https://github.com/Snow1226/CameraPlus) に、曲別カメラスクリプト機能を追加する BSIPA プラグイン群です。

譜面フォルダ内の MovementScript JSON、または `UserData/CameraSongScript/SongScripts` に集約したスクリプトを自動検出し、曲開始時にカメラへ適用します。さらに、共通スクリプト、プレビュー再生、BetterSongList 連携、HttpSiraStatus 連携、未導入譜面の BeatSaver ダウンロードまでをひとまとめに扱えます。

このソリューションに含まれる `manifest.json` は `Beat Saber 1.42.0` を対象にしています。

## ソリューション構成

現在の `CameraSongScript.slnx` は、以下の 6 プロジェクトで構成されています。

| プロジェクト | 出力 | 役割 |
|------|------|------|
| `CameraSongScript` | `CameraSongScript.dll` | 本体。SongScript 検出、UI、設定、CommonScript、プレビュー、キャッシュ、BeatSaver ダウンロード、CameraMod 自動判定を担当 |
| `CameraSongScript.Cam2` | `CameraSongScript.Cam2.dll` | Camera2 アダプタ。Camera2 SDK 経由でカメラ制御、Custom Scene 切り替え、プレビュー用マテリアル取得を担当 |
| `CameraSongScript.CamPlus` | `CameraSongScript.CamPlus.dll` | CameraPlus アダプタ。CameraPlus へスクリプトパスを渡し、プロファイル切り替えとプレビュー用マテリアル取得を担当 |
| `CameraSongScript.BetterSongList` | `CameraSongScript.BetterSongList.dll` | BetterSongList 連携。SongScript の有無でのフィルタと並び替えを追加 |
| `CameraSongScript.HttpSiraStatus` | `CameraSongScript.HttpSiraStatus.dll` | HttpSiraStatus 連携。プレイ開始時の SongScript 情報を外部へ送信 |
| `CameraSongScript.Package` | `CameraSongScript.Package.dll` | 配布用パッケージプロジェクト。Release ビルド時に本体と各アダプタをまとめた ZIP を生成 |

## 主な機能

- Camera2 / CameraPlus を自動検出し、見つかった方へ SongScript を適用します
- 譜面フォルダ内の `.json` と、`SongScripts` フォルダ内の `.json` / `.zip` を自動スキャンします
- `metadata.mapId` / `metadata.hash` に加え、ファイル名やフォルダ名先頭の 1〜6 桁の 16 進文字列からも `mapId` を推定できます
- `CommonScripts` フォルダの汎用スクリプトを、フォールバックまたは常時強制で利用できます
- 曲・難易度・Characteristic ごとのスクリプト選択と、スクリプト単位の高さオフセット保存に対応しています
- メニュー上でスクリプトの 3D プレビュー再生、シーク、倍速再生、表示位置調整ができます
- `SongScripts` に含まれる `mapId` のうち未導入譜面を BeatSaver からダウンロードできます
...
- UI ホバーヒントは `Auto` / `English` / `Japanese` を切り替え可能です
...
- SongCore
- SongDetailsCache
- Camera2 または CameraPlus のいずれか 1 つ
...
リリース ZIP には BetterSongList / HttpSiraStatus / Camera2 / CameraPlus 向けのアダプタ DLL も同梱されます。対応先 Mod が入っていない場合、それらのアダプタは `dependsOn` により自動的に無効化されます。
...
1. `metadata.mapId`
2. `metadata.hash`
3. ファイル名先頭の 1〜6 桁の 16 進文字列
4. ZIP 内親フォルダ名先頭の 1〜6 桁の 16 進文字列
5. 配置元親フォルダ名先頭の 1〜6 桁の 16 進文字列

`mapId` は BeatSaver の ID、`hash` は譜面ハッシュです。大文字小文字は区別しません。
...
1. 曲選択画面の GameplaySetup タブに `Camera Song Script` が追加されます
2. 曲を選ぶと SongScript がスキャンされ、状態・候補・metadata が表示されます
...
| `UserData/CameraSongScript/SongScripts/` | 曲別スクリプト配置フォルダ |
| `UserData/CameraSongScript/CommonScripts/` | 汎用スクリプト配置フォルダ |
...
| `StatusCanvasWidth` | `100` | ステータスパネルのキャンバス幅 |
| `StatusCanvasHeight` | `10` | ステータスパネルのキャンバス高さ |
| `StatusScale` | `0.025` | ステータスパネル全体のスケール |
...
各プリセットは以下の形式で座標と回転を持ちます。

- `Preset[Area][Slot]PosX`
- `Preset[Area][Slot]PosY`
- `Preset[Area][Slot]PosZ`
- `Preset[Area][Slot]RotX`
- `Preset[Area][Slot]RotY`
- `Preset[Area][Slot]RotZ`

例:

- `PresetLeftUpperRightPosX`
- `PresetCenterLowerLeftRotY`

`Area` は `Left` / `Center` / `Right`、`Slot` は `UpperRight` / `UpperLeft` / `LowerRight` / `LowerLeft` です。

プレビューの表示位置やスケール類も同じ `CameraSongScript.json` に保存されますが、そちらは UI の `Preview Visual Settings` から変更できます。
...
| FOV アニメーション | 対応 | CameraPlus 側で対応 |
...
CameraSongScript 本体が Camera2 SDK のトークンを使って Position / Rotation / FOV / VisibleObject を毎フレーム制御します。必要に応じて Custom Scene の切り替えも行います。

### CameraPlus

CameraSongScript は CameraPlus に SongScript パスを引き渡し、実際の MovementScript 再生は CameraPlus 側の仕組みを使います。SongScript / CommonScript 用のプロファイル切り替えも CameraPlus 側設定へ反映します。
