...
`UserData/CameraSongScript/SongScripts/`フォルダにスクリプトを配置します。スクリプトのJSON内の`metadata.mapId`とBeatSaverのID(bsr)が一致する曲で自動的に使用されます。サブフォルダでの整理やZIPファイルにも対応しています。
...
| `UserData/CameraSongScript_SongSettings.json` | 曲ごとのスクリプト選択とスクリプトごとの高さオフセット保存ファイル |
| `UserData/CameraSongScript/SongScripts/` | 曲別カメラスクリプト配置フォルダ（mapIdでマッチング） |
...
| `levelAuthorName` | マッパー名 |
| `mapId` | BeatSaverのマップID（SongScriptsフォルダでのマッチングに使用） |
| `bpm` | BPM |
| `duration` | 曲の長さ |
